...
const purchaseCart = async (req, res) => {
    ...
    try {
      ...
      if (user.cart.toString() !== cartId) {
        return res.status(401).json({ status: 'error', message: 'No tienes permisos para comprar este carrito' });
      }
      ...
      if (!cart.products.length) {
        return res.status(400).json({ status: 'error', message: 'No hay productos en el carrito' });
      }
      ...
      // Almacenar el ticket y los productos eliminados en la sesión para mostrar en la vista
      req.session.ticket = ticket;
      req.session.itemsRemoved = itemsRemoved;
      console.log(`cartcontroller:${ticket._id}`);
  
      // Redirigir a la vista del ticket
      res.json({ status: 'success', payload: { ticketId: ticket._id } });
  
    } catch (error) {
      req.logger.warning('Error durante la compra:', error);
      res.status(400).json({ status: 'error', message: error.message, error: error });
    }
};
